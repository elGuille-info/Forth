...
: VERSION-ELIZA   ( -- ) ." *** Eliza Forth v1.8 (20-ene-2023 20.44) *** " ;
...
: buscar-keyword   ( addr len -- )
  ...
  then
  \ if cr ." palabra hallada en el índice " .
...
: keyword?                             ( a1 a2 -- a1 a3 a4 f)
  ...
  if                                   ( a1 a2)
    ...
    \ s"  nokeyword " s-current debug2
  else                                 ( a1 a2)
    \ s" en keyword? en else " debug1
    \ count dup >r s-current 2swap       ( a1 a2 n2 a3 n3)
    count dup >r 
    2dup s" a ver..." 2swap debug2
    \ asignar la respuesta
    2dup answer place
    
    s-current 2swap
    \ s-current -trailing 2swap
    \ s"  s-current= " s-current -trailing  debug2
    \ search 0= nip r> swap >r           ( a1 a4 n2)
    search 0= 
    \ s" despues de search 0= " debug1
    nip r> swap >r
...
: get-answer-1                           ( -- a1 a2 n2)
  ...
  rot drop dup many choose 
  s" despues de choose " debug1
  
  cells + \ s@
  s" despues de cells + " debug1
...
: process                              \ process the input
  s-current s" BYE" starts?           \ check if input starts with "SHUT"
...
: response                             \ get a response from the user
  ...
  \ current /current -trailing current place
  \ s" en response s-current= " s-current debug2
;
