...
include eliza-en.forth
include Forth-programming-language\juegos\eliza-forth\eliza-en.forth
)

cr ." cargando el fichero eliza.forth ..." cr cr

\ Marcar que se han cargado estas palabras, 18-ene-2023 17.30
[DEFINED] eliza-en.forth [IF]
    eliza-en.forth
[THEN]
...
\ De filter.4th
\ definirlo siempre que este es el que funciona
\ [UNDEFINED] filter [IF]
: filter                               ( a n1 c -- a n2)
...
\ Está definido en util.forth, 21-ene-2023 12.32
\ De random.4th
...
\ *****************************************
\ * Los arrays y palabras del juego Eliza *
\ *****************************************
...
\ En gforth string funciona diferente que la usada en 4th, 19-ene-2023 08.51
\   En este fichero defino string según easy.4th

/current string current
/current string previous
/current string answer

\ Esto ya no vale, pero sirve para comprobar las palabras que tiene keywords
\   keywords-len   Saber cuántas palabras hay definidas
\   keyword>s      La palabra del índice indicado
\   'keyword       La dirección de memoria del índice indicado
\   'keywords      La dirección de memoria de las palabras relacionadas con el índice indicado
\   keyword.       Muestra la palabra del índice indicado
\   keywords.      Muestra todas las palabras
\   keywords?      Busca en keywords la cadena indicada
...
\ la dirección de memoria y longitud del índice indicado
...
\ la dirección de memoria del índice indicado
...
\ la dirección de memoria de las palabras relacionadas con el índice indicado
...
\ ej. s"  BEATLES " keywords?
\ si se    encuentra devuelve el índice y true
\ si no se encuentra devuelve false
: keywords?   ( addr len -- index| flag)
  false -rot
  keywords-len 0
  do
...
: keyword?                             ( a1 a2 -- a1 a3 a4 f)
  ...
  if                                   ( a1 a2)
    ...
    r> swap r>                         ( a1 a6 a5 f)
  then
  \ el top de la pila tendrá
  \     -1 si no la ha encontrado
  \      0 si la ha encontrado
  \ y la dirección de las palabras relacionadas
  \ otra dirección ???
  \ la dirección en keywords de la palabra
  \     @ count type la mostrará
;
...
\ buscar la respuesta usando el bucle de keywords
: get-answer-1
    s-current keywords?
    \ dejar la dirección de memoria, para usar después con @ count
    if 'keywords else nokeyword then
...
: get-answer                           ( -- a1 a2 n2)
  ...
  repeat                               ( a1 a2 a3)
  \ Supongo que esto dejará la memoria del array con las respuestas
  \ aquí da error 15: Invalid memory address
  \ era porque choose no elegía bien el número aleatorio
  rot drop dup many choose cells + s@  ( a2 a4 n4)
;


: process                              \ process the input
  \ un par de comprobaciones para terminar,
  \     BYE y SHUT con solo un espacio delante
...
: conversation                         \ get a proper response
  begin
    current-blank                      \ limpiar el contenido anterior
...
: eliza
  limpiar-pila                         \ borrar el contenido de la pila
  cr 4 spaces VERSION-ELIZA cr cr      \ mostrar la versión, etc
