...
: VERSION-ELIZA   ( -- ) ." *** Eliza Forth v1.2 (20-ene-2023 10.09) *** " ;
...
: s-current current-eliza count ;            ( -- a n)
...
: correct-me?                          ( a n -- a n')
  ...
  if 1- current-eliza place s" ME" current-eliza +place s-current then
;                                      \ then patch in "ME"
...
: get-answer                           ( -- a1 a2 n2)
  ...
  \ s@
  c@
  s"   en get-answer despues de c@ " debug1
  \ count
...
: process                              \ process the input
  ...
  previous-eliza place                  \ if so, exit else save response
  \ previous-eliza OVER OVER C! CHAR+ SWAP MOVE
  s" en process, previous-eliza = " previous-eliza count debug2
  get-answer                           \ scan for keywords and get an answer
  talkback                             \ now phrase the response
;

\ Guarda lo escrito por el usuario en current-eliza (s-current es lo escrito current-eliza)
: response                             \ get a response from the user
  ." > " 0 bl current-eliza over over c!     \ initialize current
  
  \ ." > " 0 bl current-eliza
  \ s"   en response despues de 0 bl current-eliza " debug1
  \ over over c!
  ...
  \ s"   en response despues de filter s-current= " current-eliza count debug2
...
: conversation                         \ get a proper response
  begin
    response s-current previous-eliza count compare 0=
  ...
  s" en conversation previous-eliza= " previous-eliza count debug2
;

: eliza
  limpiar-pila
  cr 4 spaces VERSION-ELIZA cr cr
  0 previous-eliza c!                        \ initialize previous
