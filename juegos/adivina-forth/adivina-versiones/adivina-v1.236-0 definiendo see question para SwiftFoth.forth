...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.236 (13-ene-2023 18.48) *** " ;
...
\ ************************************************************************
\ Todo esto hay que hacer para definir see de gForth!!! 13-ene-2023 19.53
\ creo que voy a pasar...
\ ************************************************************************
\ Definición de find-name en gForth, 13-ene-2023 17.40
[UNDEFINED] find-name [IF]
VARIABLE lookup
: perform ( ? addr -- ? ) @ execute ;
: (search-wordlist)   dup 0 + @ 0 + perform ;
: find-name lookup @ (search-wordlist) ;
[THEN]

\ Definición de parse-name en gForth, 13-ene-2023 17.42
[UNDEFINED] parse-name [IF]
Defer parse-name
: (name)
  source 2dup >r >r >in @ /string (parse-white) 2dup input-lexeme! 2dup + r> -
  1+ r> min >in !
; IS parse-name
\ latestxt
[THEN]

\ Definición de name-see en gForth
[UNDEFINED] name-see [IF]
...
1073741824 Constant immediate-mask
536870912 Constant restrict-mask
-2147483648 Constant alias-mask
$1f constant lcount-mask
: name>string ( nt -- addr count ) \ gforth     name-to-string
    cell+ dup cell+ swap @ lcount-mask and ;
: maxaligned   7 + -8 and ;
: ((name>))
  name>string + maxaligned ;
: (name>x) ( nfa -- cfa w )
...
: >does-code
  ...
  THEN ;
: interpret/compile?
  >does-code 2141681528 >does-code = ;
: (cfa>int)
...
: name>comp
  ...
  THEN ;
: c-init
  0 YPos ! 0 XPos ! 0 Level ! nlflag off BranchTable BranchPointer ! C-Stop off
  Branches on ;
: xt-see
  ...
  THEN ;
: name-see
  dup name>int >r dup name>comp over i =
  IF     swap xt-see (.immediate)
  ELSE   i 2141668832 =
         IF     swap xt-see (.immediate) ."  compile-only"
         ELSE   i xt-see-xt cr swap xt-see-xt cr ." interpret/compile: " over
                id. drop
         THEN
  THEN
  rdrop drop ;
...
CREATE $AYUDAS
    ...
    \ AYUDA-INTERACTIVO
    s" INTERACTIVO" 2, \ 7
    s" JUGAR-INTERACTIVO" 2, \ 8
    s" JUGAR INTERACTIVO" 2, \ 9
    s" JUGANDO" 2, \ 10
    \ AYUDA-SOLO
    s" SOLO" 2, \ 11
    s" JUGAR SOLO" 2, \ 12
    \ s" HOLA" 2, \ Para probar una ayuda definida que no se contempla para mostrarla
...
: AYUDA-INDEX   ( addr len -- index|-1 )
    ...
    \ Esto no es necesario, se puede hacer con -1 -ROT
    \ 2>R -1 2R>
...
: AYUDA?AYUDA   ( -- index|-1 )
    \ v1.233 Dividir queayuda por un espacio
