...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.144 (06-ene-2023 15.55) *** " ;
...
: LAS-DIFICULTADES-NOMBRES ( -- imprime los textos )
    %DIFICULTADES 1 + 0 do I DIFICULTAD-$ TYPE I %DIFICULTADES < IF ." , " THEN LOOP ;
...
\ Le cambio el nombre para dejarlo por si el nuevo sistema no va bien
CREATE $DIFICULTADES0
    $" _ALEATORIO" , 
    $" _SENCILLO" , 
    $" _MEDIO" , 
    $" _DIFICIL" , 
...
: $DIFICULTAD0   ( index -- addr len )
    %DIFICULTADES MIN 0 MAX 
    $DIFICULTADES SWAP CELLS + @ COUNT
;

: DIFICULTAD0-INDEX  ( addr -- index o -1 )
    %DIFICULTADES 1+ 0
    DO 
        2DUP
        I $DIFICULTAD0 COMPARE 0=
        IF 2DROP I LEAVE THEN
    LOOP
    \ Cuando llegue aquí solo debe haber un valor en la pila
    \ Si hay más es que no se ha encontrado
...
: LAS-DIFICULTADES0
    ...
    %DIFICULTADES 1 + _SENCILLO DO I . ." = " I DIFICULTAD0-$ ."  " 
