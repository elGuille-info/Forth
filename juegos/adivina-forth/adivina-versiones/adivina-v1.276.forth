...
\ 15 CONSTANT %AYUDAS
\ v1.275 ahora se calculará el tamaño al leer cuántos hay
\   Asignar inicialmente el valor -1
\ El valor asignado a %AYUDAS será 1 más del número de elementos
\   con idea de usarlo como índice superior en el bucle DO...LOOP
-1 VALUE %AYUDAS
...
\ v1.275 calcular el número elementos del array $ayudas
\   Solo se calculará la primera vez que se llame a esta palabra
: AYUDAS-LEN   ( -- n )
    %AYUDAS -1 > IF %AYUDAS EXIT THEN
    0 >R
    $AYUDAS
    BEGIN
        DUP @ NULL =
...
: AYUDA>S   ( index -- addr len )
    ...
    DUP 0< IF DROP 0 THEN AYUDAS-LEN 1- MIN
