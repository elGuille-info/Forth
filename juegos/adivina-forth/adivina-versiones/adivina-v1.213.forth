...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.213 (11-ene-2023 20.22) *** " ;
...
\ TODO:
\   Definir una palabra para comprobar todas las posibilidades al escribir ayuda.
...
: DEBUG2   ( addr1 len1 addr2 len2 -- ) 2SWAP TYPE ." '" TYPE ." ' " .s ;
...
MAX_AYUDA STRING QUENUMEROS
MAX_AYUDA STRING QUELETRAS
...
:NONAME
    ...
    \ TODO: Mostrar INSTRUCCIONES-MOSTRAR
...
CREATE $AYUDAS
    ...
    s" AYUDA JUGAR" 2,
    s" AYUDA NIVEL" 2,
    s" AYUDA DIFICULTAD" 2, \ 10
    s" AYUDA PISTA" 2, 
    s" AYUDA ADIVINA" 2,
    s" AYUDA FORTH" 2, \ 13
    s" INTERACTIVO" 2, \ 14
    s" AYUDA INTERACTIVO" 2, \ 15
    s" JUGAR-INTERACTIVO" 2, 
    s" JUGAR INTERACTIVO" 2,
    s" JUGANDO" 2, 
    s" AYUDA-JUGANDO" 2, \ 19
    \ Definir también con el guión
    \ o mejor, al buscar, quitarle el guión si no se encontró
    (
    s" AYUDA-GENERAL" 2, \ 20
    s" AYUDA-JUGAR" 2,
    s" AYUDA-NIVEL" 2,
...
: AYUDA-INDEX   ( addr len -- index|-1 )
    s" al entrar en ayuda-index " cr DEBUG
    'AYUDATMP 2! -1 'AYUDATMP 2@
    \ s" en ayuda-index despues de 'AYUDATMP 2! -1 'AYUDATMP 2@ " cr DEBUG 
    %AYUDAS 1+ 0
    \ s" en ayuda-index antes del DO, 'AYUDATMP = " 'AYUDATMP 2@ cr DEBUG2
    DO
        2DUP
        \ cr ." en ayuda-index, en el DO,  I = " I .
        \ s" antes de I AYUDA>S COMPARE 0= " DEBUG
        I AYUDA>S COMPARE 0=
        IF 
            \ Quitar la addr len y el valor -1
            \ cr ." en ayuda-index, en el IF del DO,  I = " I .
            \ s" antes de 2DROP DROP I 'AYUDATMP 2@ " DEBUG
            2DROP DROP I 'AYUDATMP 2@
            \ s"     despues de 2DROP DROP I 'AYUDATMP 2@ " cr DEBUG
            LEAVE THEN
    LOOP
    \ En la pila estará el valor del índice si se ha hallado, 
    \ si no, lo que hubiera en la pila y la dirección puesta al entrar
    2DROP
    s" al salir de ayuda-index " cr DEBUG
;

\ Comprobar si es una ayuda y si es así mostrarla
\   Por ahora solo se llama con la dirección de 'QUEAYUDA
: AYUDA-RUN   ( addr len -- index|-1 )
    \ para mostrar lo que se busca
    2DUP 2>R
    s" al entrar en ayuda-run " 2R> cr DEBUG2
    ...
    s" en ayuda-run despues del ayuda-index y DUP " cr DEBUG
    
    \ \ Si devuelve -1 es que no existe esa ayuda, devolver -1
    
    \ Si devuelve -1 es que no existe esa ayuda, buscar sin guión
    -1 = 
    IF
        \ 2R> 2DUP
        \ s" en ayuda-run no existe la ayuda " 2SWAP cr DEBUG2
        \ s" en ayuda-run no existe la ayuda " 2R> cr DEBUG2
        DROP \ quitar el -1 de la pila
        DROP \ quitar la copia hecha después del Ayuda-index anterior
        
        s" en ayuda-run no existe la ayuda " cr DEBUG
        ...
        \ QUEAYUDA-UNIR
        \ s" en ayuda-run no existe la ayuda, buscar " 'QUEAYUDA cr DEBUG2
        \ 'QUEAYUDA AYUDA-INDEX
        \ s" en ayuda-run no existe la ayuda, despues de 'QUEAYUDA AYUDA-INDEX " cr DEBUG
        
        s" en ayuda-run no existe la ayuda, buscar AYUDA " 'QUELETRAS cr DEBUG2
        cr ." en ayuda-run no existe la ayuda, 'QUENUMEROS es " 'QUENUMEROS TYPE
        'QUELETRAS AYUDA-INDEX
        s" en ayuda-run no existe la ayuda, despues de 'QUELETRAS AYUDA-INDEX " cr DEBUG
        DUP
    ELSE
        DROP \ v1.213
        s" en ayuda-run existe la ayuda: " 'QUEAYUDA cr DEBUG2
    THEN
    ...
    IF
        ...
        DUP
        s" en ayuda-run, existe la ayuda " cr DEBUG
        ...
        CASE 
            ...
            17 OF TRUE AYUDA-INTERACTIVO 16 ENDOF
...
: AYUDA-BUSCAR   ( flag -- )
    ...
    \ Esto buscará tanto ayuda xxx como xxx
    s" en ayuda-buscar antes de 'QUEAYUDA AYUDA-RUN, QUEAYUDA: " 'QUEAYUDA cr DEBUG2
    'QUEAYUDA AYUDA-RUN
    s" en ayuda-buscar despues de 'QUEAYUDA AYUDA-RUN " cr DEBUG

    DEPTH 0= 
    IF 
        CR ." ERROR debe haber algo en la pila en AYUDA-BUSCAR despues de 'QUEAYUDA AYUDA-RUN " 
        .s CR 
        -1
    THEN
    ...
    IF  
        ...
        QUEAYUDA-LEN 0>
        IF CR ." '" QUEAYUDA. ." ' no es una ayuda." THEN
        ...
        ELSE AYUDA-GENERAL
        THEN
        s" en ayuda-buscar NO existe la ayuda: " 'QUEAYUDA cr DEBUG2
    ELSE
        s" en ayuda-buscar existe la ayuda " cr DEBUG
    THEN
;
...
\   FALSE si se llama desde AYUDA
\   TRUE si se llama desde DIME
\ Aunque ya solo se llama desde DIME
\ v1.213 quito el parámetro
: AYUDA-SEE   ( -- )
    \ s" al entrar en ayuda-see " cr DEBUG
    \ DUP
    ...
    \ s" en ayuda-see QUEAYUDA: " 'QUEAYUDA cr  DEBUG2
    ...
    \ IF
        ...
        IF 
            DROP \ v1.213
            s" en ayuda-see empieza por numero " 'QUELETRAS cr DEBUG2
            'QUELETRAS COMANDO-RUN
            s" en ayuda-see despues de 'QUELETRAS COMANDO-RUN " cr DEBUG
        ELSE 
            DROP \ v1.213
            s" en ayuda-see no empieza por numero " 'QUEAYUDA cr DEBUG2
            'QUEAYUDA COMANDO-RUN
            s" en ayuda-see despues de 'QUEAYUDA COMANDO-RUN con " 'QUEAYUDA cr DEBUG2
            DUP
            ...
            -1 = 
            IF
                \ probar con QUEAYUDA-SPLIT-LETRAS-NUMEROS
                \ para buscar solo la segunda palabra, la que está en QUELETRAS
                QUEAYUDA-SPLIT-LETRAS-NUMEROS
                s" en ayuda-see, QUELETRAS= " 'QUELETRAS cr DEBUG2
                'QUELETRAS AYUDA-RUN
                s" en ayuda-see despues de 'QUELETRAS AYUDA-RUN " cr DEBUG
                
                \ s" en ayuda-see antes de QUEAYUDA-SPLIT-GUION " 'QUEAYUDA cr DEBUG2
                \ QUEAYUDA-SPLIT-GUION
                \ QUEAYUDA-UNIR
                \ s" en ayuda-see despues de QUEAYUDA-UNIR " 'QUEAYUDA cr DEBUG2
                \ -1
             THEN
        THEN
    \ ELSE
    \     s" en ayuda-see se ha llamado con false " cr DEBUG
    \     -1
    \ THEN
    ...
    IF 
        DROP \ v1.213
        s" en ayuda-see se busca en las ayudas: " 'QUEAYUDA cr DEBUG2
        TRUE AYUDA-BUSCAR THEN
...
:NONAME
    ...
    \ IF CR ." Por favor indica un numero, un comando o una ayuda." CR DIME cr ." despues de dime" THEN
    ...
    ELSE
        s" en dime no es un numero, se llama a ayuda-see " cr DEBUG
        TRUE AYUDA-SEE
    THEN
; IS DIME
...
:NONAME
    ...
    \ v1.211si no se ha escrito nada, mostrar la ayuda general
    QUEAYUDA-LEN 0= IF AYUDA-GENERAL EXIT THEN
    
    \ v1.211 buscar la ayuda, convertido a mayúsculas
    \ FALSE COMPROBARSEE !
    QUEAYUDA>MAYUSCULAS
    FALSE AYUDA-BUSCAR
    s" en ayuda, al salir despues de ayuda-buscar " cr DEBUG
    DEPTH 0 > IF s" en ayuda hay algo que borrar " cr DEBUG DROP THEN
; IS AYUDA
