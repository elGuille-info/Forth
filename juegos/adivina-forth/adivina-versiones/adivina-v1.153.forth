...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.153 (07-ene-2023 15.31) *** " ;
...
\ TODO:
\   En la ayuda de DIFICULTAD, si juega con DIME no mostrar que se puede usar D!
\       O añadirlo a los comandos.
\   En DIME cuando se indique 0 DIFICULTAD indicar que será aleatorio.
\   En DIME poder indicar la dificultad usando el nombre de la constante.
...
: QUENUMEROS-TONUMBER   ( -- numero )
    ...
    *QUENUMEROS DIFICULTAD-INDEX
    ...
    \ cr ." el valor devuelto por *QUENUMEROS DIFICULTAD-INDEX " .s
    DUP
    -1 =
    IF DROP *QUENUMEROS STR>INT 
    \ ELSE cr ." en QUENUMEROS-TONUMBER, el valor " *QUENUMEROS TYPE ."  es una dificultad " .s
    THEN
;
...
\ Deja en la pila la dirección de memoria del texto de índice indicado
: $DIFICULTAD   ( index -- addr len )
    \ %DIFICULTADES MIN 0 MAX
    %DIFICULTADES MIN _SENCILLO MAX
    \ $DIFICULTADES SWAP CELLS + @ COUNT
...
\ : DIFICULTAD-INDEX  ( addr -- index o -1 )
:NONAME
    \ cr ." al entrar en DIFICULTAD-INDEX " .s \ está la dirección de memoria y longitud
    ...
    DO 
        ...
        IF \ cr ." en DIFICULTAD-INDEX, al hallarlo " .s 
            \ 2DROP I 
            I
            -ROT
            \ cr ." el hallado en DIFICULTAD-INDEX " DUP $DIFICULTAD TYPE ." , pila " .s
            LEAVE 
        THEN
    LOOP
    \ Cuando llegue aquí solo debe haber un valor en la pila
    \ Si hay más es que no se ha encontrado
    \ DEPTH 1 > IF cr ." en DIFICULTAD-INDEX la pila " .s LIMPIAR-PILA -1 THEN
    \ los valores que había menos 2 de addr
    \ R> 2 -
    \ 1 > 
    \ IF cr ." en DIFICULTAD-INDEX la pila " .s LIMPIAR-PILA -1 
    \ else 
    \     DEPTH 2 > 
    \     IF cr ." hay mas de 2 " .s 2DROP then 
    \     \ cr ." en DIFICULTAD-INDEX, al convertir, la pila " .s 2DROP
    \ THEN
    depth R> = IF -1 -ROT THEN
    \ cr ." antes de salir de DIFICULTAD-INDEX " .s
    2DROP
; IS DIFICULTAD-INDEX
...
: MOSTRAR-NUMS ( -- )
    ...
    \ cr ." al entrar en MOSTRAR-NUMS: " .s
    DEPTH >R
    ...
    DO 
        ...
        0= 
        \ IF DROP LEAVE THEN
        IF DROP cr ." en MOSTRAR-NUMS, tras DROP antes de LEAVE: " .s LEAVE THEN
        CR
    ...
    cr ." en MOSTRAR-NUMS, antes de DEPTH R> - 0>:" .s
    DEPTH R> - 0> 
    \ Hay más datos en la pila
    IF DROP THEN \ cr ." en MOSTRAR-NUMS:" .s THEN
;
...
: COMANDO-INDEX  ( addr -- index o -1 )
    \ guardar cuántos valores hay en la pila
    \ cr ." al entrar en COMANDO-INDEX " .s
    DEPTH >R
    %COMANDOS 1+ 0
    DO 
        2DUP
    ...
    \ DEPTH 1 > IF cr ." en COMANDO-INDEX " .s LIMPIAR-PILA -1 THEN
    \ R> 2 -
    \ 1 > IF cr ." en COMANDO-INDEX " .s LIMPIAR-PILA -1 THEN
    depth R> = IF -1 -ROT THEN
    \ cr ." antes de salir de COMANDO-INDEX " .s
    2DROP
;
...
: COMANDO-RUN  ( addr -- -1 si no se halló, si no, el índice )
    \ cr ." al entrar en COMANDO-RUN " .s
    COMANDO-INDEX
    \ cr ." en COMANDO-RUN despues de COMANDO-INDEX " .s
        ...
        CASE
            ...
            0 OF 
                ...
                ELSE
                    ...
                    s" Si asignas un nuevo valor a NIVEL se reiniciara el juego."
                    CONTINUAR?
                    cr ." despues de CONTINUAR? " .s
            ...
            1 OF 
                ...
                \ cr ." en COMANDO-RUN si es disponibilidad *QUENUMEROS es " *QUENUMEROS TYPE ." , pila " .s
                \ quitar el valor de la pila, será 1, si no, QUENUMEROS-TONUMBER fallará
                \ parece que al quitar esto falla cuando es <NOMBRE> DIFICULTAD
                \ DROP
                depth 2 = if cr ." lo que habia en la pila " type then
                QUENUMEROS-TONUMBER
                DUP
                cr ." En CASE DIFICULTAD " .s
                NO_NUM =
                ...
                ELSE
                    ...
                    ELSE
                        ...
                        cr ." *quenumeros type " *quenumeros type
                        cr ." QUENUMEROS-TONUMBER = " QUENUMEROS-TONUMBER .
                        QUENUMEROS-TONUMBER 0 =
                        IF 
                            \ s" Esto asigna un nivel de DIFICULTAD aleatorio entre 1 (_SENCILLO) y 6 (_SENSEI)."
                            ." Esto asigna un nivel de DIFICULTAD aleatorio entre 1 (" 
                            1 $DIFICULTAD TYPE
                            ." ) y 6 (" %DIFICULTADES $DIFICULTAD TYPE s" )."
                        ELSE
                            \ Comprobar si es un numero menor al actual
                            \ Si el número es mayor, tendrá menos oportunidades
                            QUENUMEROS-TONUMBER NIVELDIFICULTAD @ >
                            IF 
                                \ s" Si asignas esa DIFICULTAD tendras menos intentos."
                                ." Si asignas " DUP $DIFICULTAD TYPE
                                s"  tendras menos intentos."
                            ELSE
                                s" Si asignas esa DIFICULTAD tendras mas intentos, pero es casi como hacer trampas."
                            THEN
                        THEN
                        CONTINUAR?
                        IF \ cr ." en if despues de continuar-1 " .s
                            \ SWAP DROP 
                            \ cr ." antes de asignar a dificultad " .s
                            DIFICULTAD 1 
                            \ cr ." en if despues de continuar-2 " .s
                        \ ELSE cr ." en else despues de Continuar? " .s 
                        THEN
                    THEN
                THEN
            ENDOF
...
: AYUDA-SEE
    ...
    \ Si no hay nada en la pila, asignar TRUE a COMPROBARSEE
    \ Si había algo, quitarlo y asignar FALSE a COMPROBARSEE
    DEPTH 0= IF TRUE COMPROBARSEE ! ELSE DROP FALSE COMPROBARSEE ! THEN
    
    DEPTH 0> IF  
        cr ." en AYUDA-SEE quedaba algo en la pila " .s
        LIMPIAR-PILA THEN
    ...
    IF
        ...
        *QUENUMEROS DIFICULTAD-INDEX -1 >
        \ Si es así
        IF
            \ convertir a número y dejar TRUE en la pila
            \ no hace falta convertirlo en número, se hará en COMANDO-RUN
            \ QUENUMEROS-TONUMBER
            TRUE
            \ cr ." en AYUDA-SEE es una dificultad en letras " .s
        ELSE
            QUEAYUDA-EMPIEZA-NUMERO
        THEN
...
:NONAME
    ...
    QUEAYUDA-ISNUMBER
    \ cr ." En DIME QUEAYUDA-ISNUMBER: " .s
    IF
        QUEAYUDA-TONUMBER
        DUP
        0= 
    ...
    ELSE
        ...
        cr ." Antes de entrar en AYUDA-SEE desde DIME: " .s
        AYUDA-SEE
    THEN
; IS DIME
...
:NONAME
    ...
    1 
    cr ." en AYUDA antes de entrar en AYUDA-SEE, se ha puesto el 1:" .s
    AYUDA-SEE
; IS AYUDA
