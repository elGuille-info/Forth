...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.116 (05-ene-2023 13.27) *** " ;
...
\ TODO:
\	Habría que averiguar si lo guardado en QUE.AYUDA tiene números aparte de letras.
\	Hay que cambiar STR>INT? para que devuelva false si se indica un número y letras.
\	Cambiar STR>INT para que devuelva NO-NUM si se indica un número y letras.
...
: STR>INT  \ addr len -- n 
    ...
    \ Si no lo ha convertido, devolver NO-NUM
    0= IF 2DROP NO-NUM ELSE DROP THEN
;
...
: QUEAYUDA-ISNUMBER  \ -- true o false
    \ QUE.AYUDA MAX_AYUDA -TRAILING 
    *QUEAYUDA
    STR>INT?
;
...
: AYUDA-RUN  \ addr -- -1 si no se halló, si no, el índice 
    ...
    IF
        ...
        CASE 
            ...
            8 OF AYUDA-JUGAR ENDOF
            9 OF AYUDA-NIVEL ENDOF
            10 OF AYUDA-DIFICULTAD ENDOF
            11 OF AYUDA-PISTA ENDOF
            12 OF AYUDA-ADIVINA ENDOF
            13 OF AYUDA-FORTH ENDOF
        ENDCASE
    THEN
...
: COMANDO-RUN  \ addr -- -1 si no se halló, si no, el índice 
    ...
    -1 > 
    IF
        \ cr ." antes del case en comando-run " .s cr
        \ hacer copia del índice
        DUP
        ...
        CASE 
            ...
            7 OF NUMS? ENDOF
        ENDCASE
        \ cr ." despues del case en comando-run " .s cr
    THEN
    ...
    *QUEAYUDA COMANDO-RUN
    ...
    IF LIMPIAR-PILA 
        ...
        *QUEAYUDA AYUDA-RUN
        ...
        CR ." debe haber algo en la pila " .s CR
        -1 =
        IF  CR ." La palabra indicada '" QUEAYUDA-MOSTRAR ." ' no es un comando ni una ayuda." CR
        	." *** Mostrar la ayuda segun este jugando con DIME o con JUGAR. ***" CR
        ELSE CR ." En AYUDA-SEE en la pila hay cosas si no es comando ni ayuda. " .s CR
        THEN
    THEN
...
\ Si se llama con n AYUDA-SEE no hacer nada si no se encuentra lo buscado
\ Si se llama sin nada en la pila, si no se encuentra usar 0 para AYUDA-GENERAL
: AYUDA-SEE-0
    DEPTH 0= 
    IF 
    	\ Si no se ha indicado nada en la pila convertir QUE.AYUDA en mayúsculas
    	QUEAYUDA-TOUPPER
    	TRUE COMPROBARSEE ! 
    ELSE FALSE COMPROBARSEE ! 
    THEN
    
    LIMPIAR-PILA
    \ Esto hacer solo si no se ha convertido en mayúsculas, 
    \   o no se ha llamado previamente a ADD-AYUDA
    \ QUEAYUDA-TOUPPER
    \ QUE.AYUDA MAX_AYUDA -TRAILING
    *QUEAYUDA
    %AYUDAS 1+ 0
    \ CR ." antes del do " .S CR
    DO 
        2DUP
        I $AYUDA COMPARE 0=
        IF 2DROP I LEAVE THEN
    ...
    IF
        COMPROBARSEE @ TRUE = 
        IF 0 ELSE LIMPIAR-PILA THEN 
    THEN
    
    \ Si no hay nada en la pila, salir sin hacer nada más
    \ Salvo que COMPROBARSEE tenga TRUE
    DEPTH 0= 
    IF COMPROBARSEE @ 
        IF 0 
        ELSE LIMPIAR-PILA EXIT 
        THEN 
    THEN
...
\ v1.114 esto no se utiliza, 05-ene-2023 10.33
(
\ Añadir ayuda a QUE.AYUDA
CREATE AYUDASTR MAX_AYUDA CELLS ALLOT
VARIABLE *AYUDASTR
: AYUDASTR-LENGTH   AYUDASTR COUNT SWAP DROP ;
: AYUDASTR+QUEAYUDA
	AYUDASTR 1+ MAX_AYUDA -TRAILING QUEAYUDA-LENGTH AYUDASTR-LENGTH + SWAP DROP ;
\ Añade AYUDA- al contenido de QUE.AYUDA y lo asigna a QUE.AYUDA
: ADD-AYUDA
    s" AYUDA-" AYUDASTR PLACE$
    QUEAYUDA-TOUPPER
    
    \ Mover el contenido del QUE.AYUDA a AYUDASTR después de count
    \ La dirección de AYUDASTR y los caracteres
    AYUDASTR COUNT + \ addr2+COUNT
    *AYUDASTR !
    \ La dirección final e inicio de QUE.AYUDA
    QUE.AYUDA MAX_AYUDA -TRAILING BOUNDS
    \ Esto recorre desde la addrFinal a la addrInicio
    \ y lo pone en la dirección indicada por *AYUDASTR y después incrementa *AYUDASTR
    ?DO I c@ *AYUDASTR @ c! *AYUDASTR @ 1+ *AYUDASTR ! LOOP 
    
    \ Esto muestra el nuevo contenido, 04-ene-2023 08.27
    \ AYUDASTR 1+ MAX_AYUDA -TRAILING TYPE
    \ AYUDASTR 1+ C@ EMIT \ Muestra el primer caracter
    \ AYUDASTR 10 + C@ EMIT \ Muestra el caracter de la posición 10
    
    \ Hay que usar la longitud de QUE.AYUDA + la de AYUDASTR
    \ AYUDASTR 1+ MAX_AYUDA -TRAILING QUEAYUDA-LENGTH 6 + SWAP DROP TYPE
    \ AYUDASTR 1+ MAX_AYUDA -TRAILING QUEAYUDA-LENGTH AYUDASTR COUNT SWAP DROP + SWAP DROP TYPE
    
    \ Esto contiene el contenido actual: AYUDA- más lo escrito
    \ AYUDASTR 1+ MAX_AYUDA -TRAILING QUEAYUDA-LENGTH AYUDASTR-LENGTH + SWAP DROP TYPE
    \ CR ." El contenido de AYUDASTR + QUEAYUDA: " AYUDASTR+QUEAYUDA TYPE CR
;
)
...
: DIME
    ...
    IF \ ." Es un numero"
        QUEAYUDA-TONUMBER
        0= 
        IF ." Has escrito 0, terminar." CR 
        ELSE \ ." Has escrito un numero." CR
            QUEAYUDA-TONUMBER
            A-N
        THEN
    ELSE \ ." No es un numero"
        \ Esto convierte el contenido de QUE.AYUDA en AYUDA- y lo que ya tuviera
        ADD-AYUDA
        \ por ahora solo se comprueba si el contenido de QUE.AYUDA es una de las ayudas
        LIMPIAR-PILA 1 AYUDA-SEE
    THEN
;
)
\ Preguntar por algo, para el juego adivina 03-ene-2023 20.39
: DIME
    ...
    IF \ ." Es un numero"
        QUEAYUDA-TONUMBER
        \ CR ." despues de QUEAYUDA-TONUMBER en DIME: " .s CR
        DUP
        0= 
        IF DROP ." Has escrito 0, terminar." CR 
        ELSE \ ." Has escrito un numero." CR
            \ QUEAYUDA-TONUMBER
            \ ."  antes de A-N " .s CR
            A-N
    ...
    ELSE \ ." No es un numero"
        \ v1.114 esto no se utiliza
        \ Esto convierte el contenido de QUE.AYUDA en AYUDA- y lo que ya tuviera
        \ ADD-AYUDA
        \ Si se quita ADD-AYUDA hay que convertirlo en mayúsculas
        QUEAYUDA-TOUPPER
        LIMPIAR-PILA
        \ por ahora solo se comprueba si el contenido de QUE.AYUDA es una de las ayudas
        \ 1 AYUDA-SEE
