...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.227 (12-ene-2023 19.54) *** " ;
...
MAX_AYUDA STRING QUEAYUDATMP
: 'QUEAYUDATMP   ( -- addr len ) QUEAYUDATMP MAX_AYUDA -TRAILING ;
: QUEAYUDATMP!   ( addr len -- ) QUEAYUDATMP MAX_AYUDA MAX_AYUDA ?LIMPIAR MAX_AYUDA MIN QUEAYUDATMP SWAP MOVE ;

\ v1.218 divide QUELETRAS por un espacio
\ deja en QUENUMEROS la primera parte y en QUELETRAS la segunda
\ esto parece que no va con la misma variable
: QUELETRAS-SPLIT   ( -- )
...
: AYUDA-SEE   ( -- )
    ...
    -1 =
    IF
        s" en ayuda-see NO existe la ayuda: " 'QUEAYUDA DEBUG2
        
        CR ." '" QUEAYUDA. ." ' no es una ayuda."
