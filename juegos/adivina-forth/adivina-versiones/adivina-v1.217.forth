...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.217 (12-ene-2023 08.27) *** " ;
...
: S?CHAR   ( char addr len -- pos|-1 )
    ...
    ?DO
        ...
        THEN
    LOOP
    \ s" en s?char despues de LOOP " DEBUG1
    DROP
    DUP
    \ -1 = IF cr ." en if " .s 2R> 2DROP ELSE cr ." en ELSE .s= " .s 2R> BOUNDS swap DROP - THEN
    -1 = IF 2R> 2DROP ELSE 2R> BOUNDS SWAP DROP - THEN
    \ s" al salir de s?char " DEBUG1 cr
;
\ v1.217 comprobar si QUEAYUDA tiene el carácter indicado
: QUEAYUDA?C   ( char -- flag )
    'QUEAYUDA S?CHAR
    \ s" al salir de queaayuda?c " DEBUG1
;

: test-char
    s" hola-mundo" queayuda!
    cr ." tiene el - ?"
    [char] - 'queayuda s?char
    \ cr .s
    dup 0 >= if ." si en posicion " 1 + . else drop ." no" then 
    clearstacks
    cr ." tiene la h ?"
    [char] h 'queayuda s?char
    \ cr .s
    dup 0 >= if ." si en posicion " 1 + . else drop ." no" then 
    clearstacks
    cr ." tiene el 3 ?"
    [char] 3 'queayuda s?char
    \ .s cr
    dup 0 >= if ." si en posicion " 1 + . else drop ." no" then 
    clearstacks
;
...
: AYUDA-RUN   ( addr len -- index|-1 )
    ...
    s"   en ayuda-run despues del ayuda-index y DUP " DEBUG1
    ...
    IF
        ...
        s"   en ayuda-run no existe la ayuda, despues de 'QUELETRAS AYUDA-INDEX " DEBUG1
        DUP
    ELSE
        DROP \ v1.213
    ...
    IF
        ...
        DUP
        s"   en ayuda-run, existe la ayuda " DEBUG1
...
: AYUDA-BUSCAR   ( flag -- )
    ...
    'QUEAYUDA AYUDA-RUN
    s" en ayuda-buscar despues de 'QUEAYUDA AYUDA-RUN " DEBUG1
    ...
    ELSE
        s" en ayuda-buscar existe la ayuda " DEBUG1
    THEN
;

\ v1.217 comprobar si QUEAYUDA tiene ayuda-xxx o ayuda xxx
\ Puede indicarse como ayuda xxx
: AYUDA?AYUDA   ( -- index|flag )
    \ Comprobar si es ayuda-xxx
...
: AYUDA-SEE   ( -- )
    ...
    ELSE 
        ...
        s" en ayuda-see despues de 'QUEAYUDA COMANDO-RUN con QUEAYUDA= " 'QUEAYUDA DEBUG2
        DUP
        ...
        IF
            ...
            'QUELETRAS AYUDA-RUN
            s" en ayuda-see despues de 'QUELETRAS AYUDA-RUN " DEBUG1
            ...
            if
                ...
                s" en ayuda-see despues de 'QUENUMEROS AYUDA-RUN " DEBUG1
            then
         THEN
    THEN
...
:NONAME
    ...
    ELSE
        s" en dime no es un numero, se llama a ayuda-see " DEBUG1
        AYUDA-SEE
    THEN
; IS DIME
