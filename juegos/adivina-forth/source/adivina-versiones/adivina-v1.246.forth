...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.246 (14-ene-2023 17.45) *** " ;
...
CREATE $DIFICULTADES
    ...
    $" _MAESTRO" , 
    $" _SENSEI" , 
( En SwiftForth no funciona así, o al menos no sé mostrar el contenido
\ CREATE $DIFICULTADES
    s" _ALEATORIO" 2, 
    s" _SENCILLO" 2, 
    s" _MEDIO" 2, 
...
: DIFICULTAD>N   ( index -- num )
    ...
    ( Usando esto:
        CREATE $DIFICULTADES
            ...
            s" _SENSEI" 2, 
            255 , 22 , 14 , 12 , 9 , 6 , 5 ,
        Hay que hacerlo así, funciona en gForth
...
\ ***************************************
\ v246 Todo lo relacionado con JUGAR xxx
\ ***************************************
...
: JUGAR-OPCIONES   ( -- )
    ...
    ." Escribe JUGAR seguido de una de estas opciones:" CR
    JUGAR-MOSTRAR-OPCIONES CR
    ." Si escribes solo JUGAR empezaras el juego contra el ordenador." CR
;
...
:NONAME
    ...
    \ si no se ha escrito nada, jugar en bucle
    QUEJUGAR-LEN 0= IF JUGAR-BUCLE EXIT THEN
    
    QUEJUGAR>MAYUSCULAS
    
    \ buscar si lo indicado está contemplado.
    'QUEJUGAR JUGAR-INDEX
    DUP
    -1 = IF DROP CR ." Esa opcion de JUGAR no existe." CR JUGAR-OPCIONES EXIT THEN
    CASE
...
: AYUDA-GENERAL?   ( -- )
    ...
    QUEAYUDA-LEN 0>
    IF CR ." La ayuda indicada: '" QUEAYUDA. ." ' no es una ayuda." CR THEN
