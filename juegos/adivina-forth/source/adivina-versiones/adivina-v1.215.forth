...
\ Adivinar un número
: VERSION-ADIVINA   ." *** Adivina Forth v1.215 (11-ene-2023 21.21) *** " ;
...
: DEBUG   ( addr len -- ) ESDEBUG? @ IF CR TYPE .s ELSE 2DROP THEN ;
...
: AYUDA-INDEX   ( addr len -- index|-1 )
    s"     al entrar en ayuda-index " DEBUG
    ...
    2DROP
    s"     al salir de ayuda-index " DEBUG
;
...
: AYUDA-RUN   ( addr len -- index|-1 )
    ...
    s"   en ayuda-run despues del ayuda-index y DUP " DEBUG
    ...
    IF
        ...
        s"   en ayuda-run no existe la ayuda " DEBUG
        ...
        s"   en ayuda-run no existe la ayuda, buscar AYUDA " 'QUELETRAS DEBUG2
        s"   en ayuda-run no existe la ayuda, 'QUENUMEROS es: " 'QUENUMEROS DEBUG2
        'QUELETRAS AYUDA-INDEX
        s"   en ayuda-run no existe la ayuda, despues de 'QUELETRAS AYUDA-INDEX " DEBUG
        DUP
    ELSE
        DROP \ v1.213
        s"   en ayuda-run existe la ayuda: " 'QUEAYUDA DEBUG2
    THEN
    ...
    IF
        ...
        DUP
        s"   en ayuda-run, existe la ayuda " DEBUG
...
: AYUDA-BUSCAR   ( flag -- )
    ...
    \ Esto buscará tanto ayuda xxx como xxx
    s" en ayuda-buscar antes de 'QUEAYUDA AYUDA-RUN, QUEAYUDA: " 'QUEAYUDA DEBUG2
    'QUEAYUDA AYUDA-RUN
    s" en ayuda-buscar despues de 'QUEAYUDA AYUDA-RUN " DEBUG
    ...
    IF  
        ...
        ELSE AYUDA-GENERAL
        THEN
        s" en ayuda-buscar NO existe la ayuda: " 'QUEAYUDA DEBUG2
    ELSE
        s" en ayuda-buscar existe la ayuda " DEBUG
    THEN
;
...
: AYUDA-SEE   ( -- )
    s" al entrar en ayuda-see " DEBUG
    ...
    IF 
        \ v1.214 DROP \ v1.213
        s" en ayuda-see empieza por numero " 'QUELETRAS DEBUG2
        'QUELETRAS COMANDO-RUN
        s" en ayuda-see despues de 'QUELETRAS COMANDO-RUN " DEBUG
    ELSE 
        \ v1.214 DROP \ v1.213
        s" en ayuda-see no empieza por numero " 'QUEAYUDA DEBUG2
        'QUEAYUDA COMANDO-RUN
        s" en ayuda-see despues de 'QUEAYUDA COMANDO-RUN con " 'QUEAYUDA DEBUG2
        DUP
        ...
        IF
            ...
            s" en ayuda-see, QUELETRAS= " 'QUELETRAS DEBUG2
            'QUELETRAS AYUDA-RUN
            s" en ayuda-see despues de 'QUELETRAS AYUDA-RUN " DEBUG
            
            DUP \ v1.214
            \ v1.214 si es -1 buscar lo que haya en quenumeros
            -1 =
            if
                s" en ayuda-see, QUENUMEROS= " 'QUENUMEROS DEBUG2
                'QUENUMEROS AYUDA-RUN
                s" en ayuda-see despues de 'QUENUMEROS AYUDA-RUN " DEBUG
            then
         THEN
    THEN
    ...
    IF 
        \ v1.214 DROP \ v1.213
        s" en ayuda-see se busca en las ayudas: " 'QUEAYUDA DEBUG2
...
:NONAME
    ...
    ELSE
        s" en dime no es un numero, se llama a ayuda-see " DEBUG
        \ TRUE AYUDA-SEE
        AYUDA-SEE
    THEN
; IS DIME
