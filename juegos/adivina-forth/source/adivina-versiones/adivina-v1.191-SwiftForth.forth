...
\ Adivinar un número
\ : VERSION-ADIVINA   ." *** Adivina Forth v1.191 (09-ene-2023 21.00) *** " ;
: VERSION-ADIVINA   ." *** Adivina Forth v1.191 (14-ene-2023 09.23) *** " ;

\ v1.191 (14-ene-2023 09.23)
\   Con estos cambios ya funciona en SwiftForth.
...
: DIGITO?-ant  ( nchar -- flag )
    DUP 
    48 < IF DROP FALSE EXIT THEN
...
: QUENUMEROS-LENGTH   ( -- ) *QUENUMEROS SWAP DROP ;
: QUELETRAS-LENGTH   ( -- ) *QUELETRAS SWAP DROP ;
...
\ Divide el contenido de QUE.AYUDA por un guión
\   para comprobar si se escribe AYUDA-XXX
: QUEAYUDA-SPLIT-GUION   *QUEAYUDA [CHAR] - $SPLIT QUELETRAS! QUENUMEROS! ;

VARIABLE QUEAYUDA.POS

\ Unir lo que hay en QUELETRAS QUENUMEROS y guardarlo en QUE.AYUDA
\ para usar como *QUENUMEROS *QUELETRAS
: QUEAYUDA-UNIR   ( -- )
    QUE.AYUDA QUEAYUDA.POS !
    *QUENUMEROS BOUNDS ?DO I c@ QUEAYUDA.POS @ c! QUEAYUDA.POS @ 1 + QUEAYUDA.POS ! LOOP
    s"  " drop c@  QUEAYUDA.POS @ c! QUEAYUDA.POS @ 1 + QUEAYUDA.POS !
    *QUELETRAS BOUNDS ?DO I c@ QUEAYUDA.POS @ c! QUEAYUDA.POS @ 1 + QUEAYUDA.POS ! LOOP
;
...
: LAS-DIFICULTADES-NOMBRES ( -- imprime los textos )
    %DIFICULTADES 1 + 1 DO I $DIFICULTAD TYPE I %DIFICULTADES < IF ." , " THEN LOOP ;
...
\ Esto no hace falta, no da error, 14-ene-2023 09.23
\ VARIABLE DIFICULTADDEPTH
...
:NONAME
    ...
    cr ." en DIFICULTAD-INDEX despues de LOOP " .s
    DEPTH R> = IF -1 -ROT THEN
    \ DEPTH DIFICULTADDEPTH @ = IF -1 -ROT THEN
    cr ." en DIFICULTAD-INDEX antes de 2DROP al salir " .s
    2DROP
; IS DIFICULTAD-INDEX
...
: DIFICULTAD!   ( n -- )
    \ s" al entrar en DIFICULTAD! " cr DEBUG
    ...
    \ s" al salir de DIFICULTAD! " cr DEBUG
;
...
: NIVEL   ( n|-1| -- ) 
    \ s" al entrar en nivel " cr DEBUG
    DEPTH 0= IF -1 ELSE DUP THEN
    \ Asignar el nivel si es 0 o mayor
    0 >= 
    \ IF ELNIVEL ! TRUE s" En nivel antes de reiniciar " cr DEBUG REINICIAR 
    IF ELNIVEL ! TRUE REINICIAR 
    ELSE VER-NIVELES 
    THEN
;
...
:NONAME
    \ s" al entrar en reiniciar " cr DEBUG
    ...
    THEN
    LIMPIAR-PILA
    \ s" al salir de reiniciar " cr DEBUG
; IS REINICIAR
...
: AYUDA-SEE   ( flag -- )
    ...
    IF
        ...
        cr ." QUENUMEROS es: " *QUENUMEROS TYPE ." , QUENUMEROS.len: " QUENUMEROS-length .
        cr ." QUELETRAS es: " *QUELETRAS TYPE ." , QUELETRAS.len: " queletras-length .
        ...
        *QUENUMEROS DIFICULTAD-INDEX -1 >
        ...
        ELSE 
            *QUEAYUDA COMANDO-RUN
            ...
            s" en ayuda-see, si no empieza por numero, despues de *QUEAYUDA COMANDO-RUN " cr DEBUG
            ...
            -1 = 
            IF
                cr ." la palabra escrita es " *QUEAYUDA TYPE
                QUEAYUDA-SPLIT-GUION
                QUEAYUDA-UNIR
                cr ." la palabra nueva es " *QUEAYUDA TYPE
                -1
             THEN
        THEN
        s" en ayuda-see despues de empieza por numero? " cr DEBUG
    ELSE
        -1
    THEN
    
    \ s" en ayuda-see despues del then que comprueba el parametro " cr DEBUG
    
    \ Si es -1 es que no está esa palabra, buscar en las ayudas
    -1 = 
    IF
        \ s" en ayuda-see antes de llamar a *queayuda ayuda-run " cr DEBUG
        cr ." contenido de QUEAYUDA " *QUEAYUDA TYPE
